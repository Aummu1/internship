...
class MyApp(MDApp):
    ...
    #ฟังก์ชันนี้จะทำการสแกนอุปกรณ์ Bluetooth ที่มีชื่อในรายการ anchors, คำนวณระยะทางจากค่า RSSI, และหาตำแหน่งของอุปกรณ์ที่ไม่ทราบตำแหน่งโดยใช้ Trilateration เมื่อมีระยะทางครบ 3 ค่า
    #ผลลัพธ์จะถูกแสดงใน UI และบันทึกใน log เพื่อการตรวจสอบ.
    def display_scan_results(self, devices):
        self.scanned_devices = []  # ล้างข้อมูลอุปกรณ์ที่สแกนก่อนหน้านี้
        scanned_info = '' # ตัวแปรสำหรับเก็บข้อมูลที่จะแสดงผล
        n = 2

        # RSSI_0 values for each anchor
        RSSI_0_values = { # กำหนดค่า RSSI ที่ระยะ 1 เมตร สำหรับแต่ละ anchor ที่ใช้ในระบบ
            "P2N_09714": -51,
            "P2N_09725": -54,
            "ZLB_39612": -51
        }
        ...
        distances = [] # รายการระยะทางที่คำนวณจาก RSSI
        rssi_values = [] # รายการค่า RSSI ที่ได้รับจากแต่ละอุปกรณ์
        positions = [] #รายการตำแหน่ง (x, y) ของ anchors

        # ฟังก์ชันจะวนลูปผ่านแต่ละอุปกรณ์ในรายการ devices ที่ได้รับมา
        for device in devices:
            name = device.name
    ...
    # แปลงพิกัดในหน่วย "จริง" (real-world coordinates) เช่น เมตร ให้กลายเป็นพิกัดในหน่วย "พิกเซล" (pixels)
    ...
    def send_data(self):
        ...
        # ใช้สำหรับส่งข้อมูลของอุปกรณ์ไปยัง API ที่ระบุ โดยผ่าน HTTP POST พร้อมจัดการข้อผิดพลาดที่อาจเกิดขึ้นระหว่างการเชื่อมต่อ
