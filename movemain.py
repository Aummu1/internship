...
class MovingWaveShadowWidget(Widget):
    def __init__(self, **kwargs):
        ...
        self.x_pos = 800  # ตำแหน่งเริ่มต้นของวงกลม
        self.y_pos = 500
        self.speed_x = 2  # ความเร็วในการเคลื่อนที่ในแนวนอน
        self.speed_y = 1  # ความเร็วในการเคลื่อนที่ในแนวตั้ง
        self.time = 0  # สำหรับการสร้างคลื่น
        ...
        with self.canvas:
            ...
            # เพิ่มการแปลงตำแหน่งให้กับวงกลม
            self.translation = Translate(self.x_pos, self.y_pos)

            # เพิ่มวงกลมที่คุณกำหนดไว้
            self.add_circle_layers()

        # ตั้งค่าให้ฟังก์ชันอัปเดตตำแหน่งถูกเรียกเป็นระยะ
        Clock.schedule_interval(self.update, 1 / 60)

    def add_circle_layers(self):
        # เพิ่มวงกลม 3 ชั้นด้วยสีตามที่คุณระบุ
        with self.canvas:
            # ชั้นที่ 1
            Color(142 / 255, 174 / 255, 219 / 255)
            vertices_outer = self.create_circle_mesh(50)
            Mesh(
            ...
            # ชั้นที่ 2
            Color(114 / 255, 151 / 255, 207 / 255)
            vertices_middle = self.create_circle_mesh(35)
            Mesh(
            ...
            # ชั้นที่ 3
            Color(84 / 255, 125 / 255, 189 / 255)
            vertices_inner = self.create_circle_mesh(20)
            Mesh(
    ...
    def create_circle_mesh(self, radius, num_points=50):
        # สร้าง vertices สำหรับวงกลม
        vertices = []
